...
import java.lang.*;
public class monster3 extends main implements Runnable{
			...
			if((attack&&lastAttackEnd)||skill){
				...
				if(( turnRightOrLeft&&( (jCharacter.getX()+250<=monsterIcon.getX()+80) && (jCharacter.getX()+250>=monsterIcon.getX())) &&  jCharacter.getY()==440)||skill)
				{
					if(attack&&!skill){
						...
						monster3_damege.setIcon(damageIcon);
						monster3_damege.setLocation(monsterIcon.getX()-130,monsterIcon.getY()-230);
						monster3_damege.setVisible(true);
					}
					if(skill){
						damageIcon=new ImageIcon("image\\critizen.gif");
						monster3_damege.setIcon(damageIcon);
						monster3_damege.setLocation(monsterIcon.getX()-130,monsterIcon.getY()-250);
						monster3_damege.setVisible(true);
					}
					lastAttackEnd=false;
					if(health==1){ //death 			
