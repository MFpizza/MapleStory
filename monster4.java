...
import java.lang.*;
public class monster4 extends main implements Runnable{
			...
			if((attack&&lastAttackEnd)||skill){
				...
				if(( turnRightOrLeft&&( (jCharacter.getX()+250<=monsterIcon.getX()+80) && (jCharacter.getX()+250>=monsterIcon.getX())) &&  jCharacter.getY()==440)||skill)
				{
					if(attack&&!skill){
						...
						monster4_damege.setIcon(damageIcon);
						monster4_damege.setLocation(monsterIcon.getX()-130,monsterIcon.getY()-230);
						monster4_damege.setVisible(true);
					}
					if(skill){
						damageIcon=new ImageIcon("image\\critizen.gif");
						monster4_damege.setIcon(damageIcon);
						monster4_damege.setLocation(monsterIcon.getX()-130,monsterIcon.getY()-250);
						monster4_damege.setVisible(true);
					}
					lastAttackEnd=false;
					if(health==1){ //death 			
